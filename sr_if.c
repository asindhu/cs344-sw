...
void sr_add_interface(struct sr_router* sr, const char* name)
{
    struct sr_if* if_walker = 0;
...
/*--------------------------------------------------------------------- 
 * Method: sr_add_iface_from_vns(..)
 * Scope: Global
 *
 * Add and interface to the router's list from a VNS-defined struct
 * (a struct sr_vns_if as defined in sr_base_internal.h)
 *
 *---------------------------------------------------------------------*/
...
void sr_add_iface_from_vns(struct sr_router* sr, struct sr_vns_if* vns_iface)
{
	...
	/* Assign values */
    strncpy(if_walker->name,vns_iface->name,sr_IFACE_NAMELEN);	// interface name
	memcpy(if_walker->addr,vns_iface->addr,ETHER_ADDR_LEN);		// interface eth addr
	if_walker->ip = vns_iface->ip;
	if_walker->mask = vns_iface->mask;
	if_walker->speed = vns_iface->speed;
...
void sr_set_ether_addr(struct sr_router* sr, const unsigned char* addr)
{
    struct sr_if* if_walker = 0;
...
void sr_set_ether_ip(struct sr_router* sr, uint32_t ip_nbo)
{
    struct sr_if* if_walker = 0;
...
void sr_print_if(struct sr_if* iface)
{
    ...
    fprintf(stderr, "%s\tHWaddr ",iface->name);
    printEthAddr(iface->addr);
    fprintf(stderr, "\n");
