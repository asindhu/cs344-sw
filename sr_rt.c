...
int sr_load_rt(struct sr_router* sr,const char* filename)
{
    FILE* fp;
    char  line[BUFSIZ];
    char  dest[32];
...
void sr_add_rt_entry(struct sr_router* sr, struct in_addr dest,
...
void sr_print_routing_table(struct sr_router* sr)
{
    struct sr_rt* rt_walker = 0;
